...
namespace MyCompany.Services
{
                ...
                if (_defaultSettings == null)
                {
                    ...
                    if (_defaultSettings == null)
                    {
                        ...
                        if (_defaultSettings == null)
                        {
                            ...
                            EnsureJsonProperty(ui, "theme.accent", "Grapello");
    ...
    public class ApplicationSessionStateBase : SessionStateStoreProviderBase
    {
        ...
        {
            using (var cmd = CreateSqlText("DELETE FROM aspnet_Sessions WHERE Expires < @Expires"))
            {
        ...
        public override void SetAndReleaseItemExclusive(HttpContext context, string id, SessionStateStoreData item, object lockId, bool newItem)
        {
            ...
            if (newItem)
            {
                ...
                using (var cmd = CreateSqlText("INSERT INTO aspnet_Sessions (SessionId, ApplicationName, Created, Expires, LockDa" +
            	...
            	using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Expires = @Expires, SessionItems = @SessionItems, Lock" +
        ...
        private SessionStateStoreData GetSessionStoreItem(bool lockRecord, HttpContext context, string id, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actionFlags)
        {
            ...
            try
            {
                ...
                if (lockRecord)
                	using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Locked = @LockedTrue, LockDate = @LockDate WHERE Sessi" +
                ...
                using (var cmd = CreateSqlText("SELECT Expires, SessionItems, LockId, LockDate, Flags, Timeout FROM aspnet_Sessio" +
                ...
                if (deleteData)
                	using (var cmd = CreateSqlText("DELETE FROM aspnet_Sessions WHERE SessionId = @SessionId AND ApplicationName = @A" +
                            "pplicationName"))
                    {
                        cmd.AddParameter("SessionId", id);
                        cmd.ExecuteNonQuery();
                ...
                {
                    lockId = (((int)(lockId)) + 1);
                    using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET LockId = @LockId, Flags = 0 WHERE SessionId = @Session" +
        ...
        {
            var cmd = new SqlText(sql, _connectionStringName);
        ...
        {
            using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Locked = 0, Expires = @Expires WHERE SessionId = @Sess" +
        ...
        {
            using (var cmd = CreateSqlText("UPDATE aspnet_Sessions SET Expires = @Expires WHERE SessionId = @SessionId AND Ap" +
